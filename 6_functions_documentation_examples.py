...
# Al hacer la llamada a la funcion ahí pasamos los numeros como argumentos
suma(10)                                                                    #Al pasar un solo numero como argumento da un error TypeError                  
...
def resta(num1=None, num2=None):                           #Los valores despues del signo = son los que definimos por defecto
...
# le pasa a la funciónson copias de los valores y no las variables en sí.
...
# Al trabajar con valores mutables se comportan como paso por referencia ya que conservan ya que conservan los cambios hechos dentro de la funcion.
...
lambda_doble_asterisco = lambda **kwargs: args[0] # Esta funcion tiene un numero indeterminado de parametros y devuelve el valor de la primera posicion
...
# Funciones anidadas
''' 
    En Python las funciones se encuentran al mismo nivel que al mismo nivel que otros objetos de Python como enteros, cadenas, módulos, etc. 
...
# El closure funciona cerrando la variable local en el momento de jecucion, pero que se mantiene después de que la ejecucion de la función haya terminado de ejecutarse.
